...
C     DEPLACEMENT TOTAL COURANT (DEPPLU) : 'PDEPL_P'
      ...
      ELSE
        ...
        CZMFE = 0.D0
      ENDIF
      IF (NINT(RELA).EQ.1.OR.NINT(RELA).EQ.2) THEN
...
C     DEFINITIONS DIFFERENTES SELON LA LOI COHESIVE
     ...
     &              NOMRES,VALRES,CODRET, 'FM' )
        IF(NINT(RELA).EQ.1) LC=VALRES(1)/VALRES(2)
        IF(NINT(RELA).EQ.2) LC=2.D0*VALRES(1)/VALRES(2)
        ALPHA0=VALRES(3)*LC        
      ENDIF
...
C          ET LA NORMALE ND ORIENTÉE DE ESCL -> MAIT
     ...
     &        CFACE,IPGF,NNO,IGEOM,JBASEC,G,RBID,FFP,FFPC,
     &        DFBID,ND,TAU1,TAU2)
           ELSEIF (NDIM.EQ.2) THEN
              CALL XJACF2(ELREF,ELREFC,ELC,NDIM,FPG,JPTINT,IFA,
     &               CFACE,NPTF,IPGF,NNO,IGEOM,JBASEC,G,
