...
!-----------------------------------------------------------------------
    ...
    integer :: imate, idimge, npara, nno, igeom, ndim, ino, ibid
...
!-----------------------------------------------------------------------
    ...
    lparam2=(/ 'PPRESSR', 'PPRESSF', 'PFR2D3D', &
...
!   =========================================================================
        do kpara=1,2
            param=lparam1(kpara)
            call tecach('NNN', param, 'L', 8, itab, iret)
            if (iret.eq.0) then
                jad=itab(1)
                nbv=itab(2)
                call assert(itab(5).eq.1 .or. itab(5).eq.4)
                if (itab(5).eq.1) then
                    do k=1,nbv
...
!       -- on regarde d'abord theta :
        call tecach('OOO', 'PTHETAR', 'L', 8, itab, iret)
...
!       -- le resultat est nul si les coefficients d'amortissement sont nuls ou absents.
        ...
        call elref4(' ', 'RIGI', ndim, nno, ibid, ibid, ibid,&
                    ibid, ibid, ibid)
        call tecach('ONN', 'PGEOMER', 'L', 5, itab, iret)
        igeom=itab(1)
        idimge=itab(2)/nno
