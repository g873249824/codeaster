...
!
    if (exivrc) then
        call jelira(chmat// '.CVRCVARC', 'LONMAX', nvacr, k8b)
...
!       seules relations de type COMP_INCR autorisees
        ...
        if (.not.lldcok) call u2mesk('F', 'RUPTURE1_69', 1, k16ldc)
!
!       on interdit VMIS_ISOT_TRAC en presence de thermique
        if (k16ldc .eq. 'VMIS_ISOT_TRAC  ' .and. exitem) call u2mess('F', 'RUPTURE1_70')
    enddo
!
    call jedema()
!
