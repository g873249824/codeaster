...
!
    ...
    character(len=8) :: chmat, k8b, other_varc, kbid
...
!
    call rsadpa(resu, 'L', 1, 'CHAMPMAT', iord0, 0, jadmat, k8b)
    chmat = zk8(jadmat)
...
!
        other_varc = ''
        call jelira(chmat// '.CVRCVARC', 'LONMAX', nvacr , kbid)
        call jeveuo(chmat// '.CVRCVARC', 'L', cvrcvarc)
        do ivarc = 1, nvacr
            if ( zk8(cvrcvarc) .eq. 'TEMP    ' ) then
                exitem = .true.
            else
                other_varc = zk8(cvrcvarc)
            endif
        enddo
!
!        if (other_varc.ne.'') call u2mess('F', 'RUPTURE1_72', nk=2, valk=[other_varc,chmat])

        if (other_varc.ne.'') call u2mess('F', 'RUPTURE1_72')
!
    endif
!
