      ...
      IMPLICIT   NONE
C ----------------------------------------------------------------------
C MODIF PREPOST  DATE 10/04/2013   AUTEUR LADIER A.LADIER 
...
C
      ...
      INTEGER NUMEMO, NBMODL, NMAIL,NDIM,IFIMED,CODRET
      INTEGER IUTIL, IDEBU
...
C
      CHARACTER*1  CECR,K1BID,K1OCC,SAUX01
      ...
      CHARACTER*8  LERESU,NOPASE,NOMGD,SAUX08
...
C
      ...
      LOGICAL LMAIL,AFAIRE,LREST,LNCMED,EXISTM
C
C ----------------------------------------------------------------------
C
      CALL JEMARQ()
...
C               (NBNOS = NOMBRE DE NOEUDS SOMMETS DE CETTE LISTE)
               ...
               IF ( NBNOS.EQ.0 ) CALL U2MESS('F','PREPOST5_9')
...
C          - ECRITURE DU CONCEPT LERESU SUR FICHIER FICH AU FORMAT FORM
     ...
     &         ZK16(JPARA),NBORDR,ZI(JORDR),LRESU,'RESU',IOCC,MODELE,
     &         CECR,TYCHA,LCOR,NBNOT,ZI(JNUNOT),NBMAT,ZI(JNUMA),NBCMP,
