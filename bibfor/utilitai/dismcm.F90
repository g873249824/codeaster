...
!
!
    character(len=8) :: kbid, mater, nomf, novarc, ktyp
...
!     --------------------------------------
        ...
        if (iret .ne. 0) then
            ...
            call jelira(nomob//'.CVRCVARC', 'LONMAX', nbvarc, kbid)
            do kvarc=1,nbvarc
                ...
                call jelira(nomcar2//'.VALE', 'LONMAX', n1, kbid)
                n2=n1/nedit
                call assert(n1.eq.nedit*n2)
                do kedit=1,nedit
                    ktyp=zk16(jvale-1+(kedit-1)*n2+2)
                    call assert(ktyp.eq.'EVOL' .or.ktyp.eq.'CHAMP')
