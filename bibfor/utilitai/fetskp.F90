...
!
        ...
        if (meth .eq. 'PMETIS  ') then
            call gtoptk('prog:pmetis', jnom(1), iret)
            if (iret .ne. 0) then
                vali(1) = len(rep)
                call utmess('F', 'EXECLOGICIEL0_24', sk='pmetis', si=vali(1))
            endif
        else if (meth .eq. 'KMETIS  ') then
            call gtoptk('prog:kmetis', jnom(1), iret)
            if (iret .ne. 0) then
                vali(1) = len(rep)
                call utmess('F', 'EXECLOGICIEL0_24', sk='kmetis', si=vali(1))
            endif
         endif
