...
# ------------------------------------------------------------------------------
  ...
  def Initialise_dparam(self):
    ...
    dparam={
      ...
      'machine_salome_runscript' : { 'label': _(u"Lanceur runSalomeScript"),       'val': os.path.join(aster.repout(), 'runSalomeScript'),             'type': 'texte',          'section': 'CONFIG',          'mode_graphique': ['Salome'],                   'mode': ['LOCAL', 'DISTANT'],        'bulle': _(u"Chemin vers le script runSalomeScript (dans l'arborescence de Salome)."), },
...
# ------------------------------------------------------------------------------
  ...
  def Sauvegarder(self, fichier, interface):
    ...
    if res:
       self.Saved = True
       txt = _(u"Nouveaux parametres sauvegardés dans : " + fichier)
       UTMESS('I','STANLEY_28',valk=[fichier])
       interface.ligne_etat.Affecter( txt )
    else:
       txt =  _(u"Impossible de sauvegarder les parametres dans : " + fichier)
