...
# ==============================================================================
    MASSIF = FACT(statut='f',max='**',
        ...
        ANGL_EULER = SIMP(statut='f',typ='R',min=1,max=3,fr=tr("Un angle en 2D, 3 angles en 3D.")),
        ANGL_AXE   = SIMP(statut='f',typ='R',min=2,max=2),
        ORIG_AXE   = SIMP(statut='f',typ='R',min=2,max=3,fr=tr("2 valeurs en 2D, 3 valeurs en 3D.")),
    ),
#
# ==============================================================================
...
# ==============================================================================
    GRILLE = FACT(statut='f',max='**',
        ...
        ANGL_REP        = SIMP(statut='f',typ='R',min=2,max=2, fr=tr("Angles de rotation permettant de définir l'axe local x.") ),
        EXCENTREMENT    = SIMP(statut='f',typ='R'),
        EXCENTREMENT_FO = SIMP(statut='f',typ=(fonction_sdaster,nappe_sdaster,formule) ),
        AXE             = SIMP(statut='f',typ='R',min=3,max=3, fr=tr("La projection de ce vecteur sert à définir l'axe local y.")),
...
# ==============================================================================
   MEMBRANE = FACT(statut='f',max='**',
      ...
      ANGL_REP = SIMP(statut='f',typ='R',min=2,max=2, fr=tr("Angles de rotation permettant de définir l'axe local x.") ),
      AXE      = SIMP(statut='f',typ='R',min=3,max=3, fr=tr("La projection de ce vecteur sert à définir l'axe local y.")),
      N_INIT   = SIMP(statut='f',typ='R',defaut=0.0,val_min=0.0, fr=tr("Pré-tension initiale ; donnée utilisée uniquement dans le cas non-linéaire.")),
   ),
#
# ==============================================================================
